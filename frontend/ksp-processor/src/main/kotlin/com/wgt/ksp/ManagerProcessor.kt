...
class ManagerProcessor(
    ...
    /**
     * 生成所有 Manager 的聚合初始化函数（可选）
     */
    private fun generateInitAllManagersFunction() {
        if (managers.isEmpty()) return

        // 找出最合适的包名（使用第一个 Manager 的包名作为基础）
        val basePackage = managers.first().packageName
        
        val fileSpec = FileSpec.builder(
            packageName = "com.wgt.architecture.manager.generated",
            fileName = "InitAllManagers"
        ).apply {
            addFileComment("Auto-generated by KSP. Do not modify.")
            
            val funBuilder = FunSpec.builder("initAllManagers")
                .addKdoc(
                    """
                    Initialize all registered Managers.
                    
                    This function is called by InitManager() to register all @ManagerProvider annotated Managers.
                    """.trimIndent()
                )
            
            managers.forEach { manager ->
                funBuilder.addStatement("${manager.initFunctionName}()")
                addImport(manager.packageName, manager.initFunctionName)
            }
